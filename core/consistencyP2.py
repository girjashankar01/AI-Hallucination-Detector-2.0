...
def _get_fact_premise(claim: str) -> str | None:
    ...
    """
    prompt = (
        f"Write one factual sentence that directly addresses the specific "
        f"assertion in this claim.\n"
        f"Rules:\n"
        f"  • Name the same person, award, event, or entity the claim mentions.\n"
        f"  • State the verified, correct detail about them "
        f"(e.g. what prize they actually won, what role they actually hold).\n"
        f"  • Do NOT say whether the claim is true or false — only state the fact.\n"
...
def _ask_bart_mnli(claim: str) -> dict | None:
    ...
    # The claim is embedded in hypothesis_template so BART still performs
    ...
    # Concatenate premise and claim so BART classifies their *relationship*
    # directly — "are these two sentences consistent or contradictory?"
    # This is the task BART-MNLI was trained on, and it handles subtle
    # factual differences (photoelectric effect vs relativity) far better
    # than the hypothesis_template approach, which required meta-reasoning
    # about abstract "supported/refuted" labels against a complex hypothesis.
    combined = f"{premise} {claim}"
    ...
    try:
        ...
        consistent_score = 0.5   # neutral default if label not found
        top_label        = data[0].get("label", "unknown")

        for item in data:
            if item.get("label", "").lower() == "consistent":
                consistent_score = item.get("score", 0.5)
                break

        # Thresholds: ≥0.6 = TRUE, ≤0.4 = FALSE, else UNCERTAIN
        verdict = "TRUE" if consistent_score >= 0.6 else ("FALSE" if consistent_score <= 0.4 else "UNCERTAIN")

        return {
            "verdict":    verdict,
            "confidence": round(consistent_score, 4),
            "reason":     (f"NLI (premise+claim concat): P(consistent)={consistent_score:.3f}, "
                           f"top='{top_label}'"),
        }
